...
void TerrainQuad::init(const QString& textureFile)
{
  ...
  QImage heightImage = QImage("../terrain2.ppm");
  ...
  for (unsigned int r = 0; r < numRows + 1; ++r) {
    for (unsigned int c = 0; c < numCols + 1; ++c) {
      ...
      // QRgb pix = heightImage.pixel((heightImage.width() - 1) * (z),
      //                              (heightImage.height() - 1) * (x));
      // float y = 0.25f * static_cast<float>(pix & 0xFF) / 255.0f;
      float y = 0.0f;
